...
DuoSlicer can host an AI assistant that talks to a **DeepSeek harness** running on a desktop machine. The assistant can read your model, act on a region you paint onto it, and - the headline capability - **turn a photograph into a printable STL that lands on the build plate by itself**.
...
```text
   phone (DuoSlicer)              desktop (harness host)          GPU box
 ┌────────────────────┐         ┌──────────────────────┐       ┌──────────────────┐
...
A three-step bootstrap that needs nothing from the user:
