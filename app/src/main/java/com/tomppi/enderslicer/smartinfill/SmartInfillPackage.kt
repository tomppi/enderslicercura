...
class SmartInfillPackageStore(private val context: Context) {
    ...
    private fun loadPackage(directory: File): SmartInfillPackage {
        ...
        return SmartInfillPackage(
            ...
            pattern = root.getString("pattern").take(MAX_LABEL),
            mode = root.getString("mode").take(MAX_LABEL),
            perimeters = perimeters,
            lineWidthMm = lineWidth,
            topBottomLayers = topBottom,
            layerHeightMm = layerHeight,
            upstreamCommit = root.getString("upstreamCommit").take(MAX_LABEL),
            modifiers = modifiers,
        )
    }
    ...
    private fun parseMetadata(raw: String): Metadata {
        ...
        return Metadata(
            ...
            baseDensityPercent = baseDensity,
            pattern = root.optString("pattern", "cubic").take(MAX_LABEL),
            mode = root.optString("mode", "graded").take(MAX_LABEL),
            perimeters = perimeters,
            lineWidthMm = lineWidth,
            topBottomLayers = topBottom,
            layerHeightMm = layerHeight,
            upstreamCommit = root.optString("upstreamCommit", "unknown").take(MAX_LABEL),
        )
    }
