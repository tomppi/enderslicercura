...
if ! have curl; then
  ...
  AUTH="$(curl -fsS --max-time 5 "${HARNESS_ORIGIN}/auth.json" 2>/dev/null || true)"
  if [ -n "${AUTH}" ]; then
    ok "harness" "${HARNESS_ORIGIN} answered auth.json"
    URL="$(printf '%s' "${AUTH}" | sed -n 's/.*"\(http[^"]*token=[^"]*\)".*/\1/p' | head -1)"
    if [ -n "${URL}" ]; then
      echo
      echo "  Paste this into the app's chat setup - it carries the launch token:"
      echo "    ${URL}"
    fi
  else
