...
with open(path, encoding="utf-8", errors="replace") as f:
    for line in f:
        ...
        if prev and p[2] is not None and prev[2] is not None:
            ...
            if d > 0.01:
                lengths.append((d, p[2] - prev[2]))
        prev = p

lengths.sort(key=lambda t: -t[0])
print("moves:", len(lengths))
print("top 10 move lengths (mm) with Z delta:")
for d, dz in lengths[:10]:
    print("  len=%.3f dz=%+.3f" % (d, dz))
long = [t for t in lengths if t[0] > 1.0]
print("moves longer than 1mm:", len(long))
if long:
    print("  max dz among those:", max(abs(t[1]) for t in long))
seg = [t[0] for t in lengths]
print("median move length: %.3f mm" % (sorted(seg)[len(seg) // 2]))
